...
function set_upload_param(up, filename)
{
    ...
    var suffixUp = suffix.toUpperCase();
    if(suffixUp===".BMP"||suffixUp===".JPG"||suffixUp===".JPEG"||suffixUp===".PNG"||suffixUp===".GIF"){
        g_dirname = 'img/';
    }else {
        g_dirname = 'video/';
    }
    ...
    init: {
        ...
        FilesAdded: function(up, files) {
            ...
            plupload.each(files, function(file) {
                document.getElementById('videolist').innerHTML = '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ')<b></b>'
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                document.getElementById("url").value=host +'/' + get_uploaded_object_name();
...
var imgUploader = new plupload.Uploader({
    ...
    init: {
        ...
        FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                document.getElementById("poster").value=host +'/' + get_uploaded_object_name();
                document.getElementById(file.id).getElementsByTagName('b')[0].innerHTML = 'success, url:' + host +'/' + get_uploaded_object_name();
            }
            else
            {
